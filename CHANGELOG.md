...
### Added
- PWS (PADS Wire System) ネットリスト形式のエクスポート機能を追加
- BD (Board Designer) ネットリスト形式のエクスポート機能を追加
- PADS Report (テキストレポート) 形式のエクスポート機能を追加
- 各CAD形式のパーサー・エクスポーターをRustで実装し、TypeScriptから統合

### Changed
- エクスポート処理を統一し、BOM A/B個別エクスポートと比較結果・置き換え結果エクスポートを同じUIで操作可能に変更
- プロジェクト管理システムを改善し、LocalStorageとの同期処理を最適化

### Technical
- 処理中フラグの二重管理問題を発見（今後の課題として記録）
  - グローバルUI処理フラグ（`setProcessing`）とReact状態フラグ（`setIsProcessing`）が併用されている
  - エクスポート処理では両方が設定されるため機能的には問題ないが、責任範囲が曖昧
...
### Added
- 比較・置換結果テーブルに列表示コントロールを追加し、Ref／部品型番／メーカー列や派生列を用途に合わせて切り替え可能にした

### Changed
- 比較モードではフィルタの選択肢を「すべて」「差分のみ」に限定し、置換モードでは従来の「追加／削除／変更」フィルタを維持するようにした
- diff 処理から Value 列専用の変更検出を削除し、全列比較のロジックへ一本化した

### Docs
- README のバージョンバッジと機能一覧を最新版に更新
- `docs/USER_GUIDE.md` にフィルタの挙動と列表示コントロールの使い方を追記

### CI
- Windows ビルドワークフローでビルド前に `src-tauri/target/release/bundle` を削除し、キャッシュ復元時の旧バージョン成果物を排除
