...
- **対応フォーマット**: CSV, XLSX, PADS ECO, CCF, MSF
...
2. 読み込み後、列役割（Ref / Part_No / Value / Comment など）が自動推定されます。誤っている場合は列ヘッダのプルダウンで修正してください。
...
- **空欄補完**: Value / Part_No の欠損を既知値で埋める
- **テキストクリーニング**: 全角／半角変換、不要な記号除去など
- **書式整形**: 表示用の太字・取り消し線・セルカラーなど

前処理は適用後に即座にデータへ反映されます。複数ブロックを組み合わせる場合は適用順序に注意してください。

## 6. 辞書管理

辞書タブでは登録名リストと例外リストを管理します。

- **登録名リスト**: `Part_No` や `Value` を指定の文字列へ置換
...
- 「全表示」をオンにすると、追加で推論された列（Value や Comment など）も含めて A/B 両方の値を一覧できます。比較対象の列を減らしたい場合は、必要な列だけオンにしてください。
